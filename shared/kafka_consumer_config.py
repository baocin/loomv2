...
class KafkaConsumerConfigLoader:
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not row:
                        logger.warning(f"No consumer configuration found for service: {service_name}")
                        return None
    ...
    def get_kafka_config(self, service_name: str, 
    ...
    def create_consumer(self, service_name: str, 
                       topics: List[str],
                       bootstrap_servers: Optional[str] = None,
                       additional_config: Optional[Dict[str, Any]] = None) -> Consumer:
        """
        ...
        """
        config = self.get_kafka_config(service_name, bootstrap_servers, additional_config)
        ...
        logger.info(f"Created consumer for {service_name} with group {config['group.id']}")
    ...
    def get_processing_recommendations(self, service_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'max_processing_time_ms': config.max_poll_interval_ms - config.session_timeout_ms,
    ...
    def _get_specific_recommendations(self, config: ConsumerConfig) -> List[str]:
        ...
        if config.max_poll_records == 1:
            recommendations.append("Process messages one at a time - likely GPU/compute intensive")
            recommendations.append("Consider implementing result caching if processing similar data")
            
        if config.max_poll_records > 500:
            recommendations.append("High batch size - ensure batch processing is implemented")
            recommendations.append("Monitor memory usage during peak loads")
            
        if config.max_poll_interval_ms > 600000:  # 10 minutes
            recommendations.append("Long processing timeout - ensure proper error handling")
        ...
        if not config.enable_auto_commit:
            recommendations.append("Manual commit enabled - commit after successful processing")
            recommendations.append("Implement proper error recovery without message loss")
