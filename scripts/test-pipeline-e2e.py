...
class PipelineTester:
    ...
    async def test_audio_ingestion(self) -> bool:
        ...
        try:
            async with self.session.post(
                f"{INGESTION_API_URL}/audio/upload", json=SAMPLE_PAYLOADS["audio"]
            ) as resp:
                if resp.status == 200:
    ...
    async def test_sensor_ingestion(self) -> bool:
        ...
        try:
            async with self.session.post(
                f"{INGESTION_API_URL}/sensor/gps", json=SAMPLE_PAYLOADS["gps"]
            ) as resp:
                if resp.status == 200:
                    result = await resp.json()
                    logger.info(
                        f"✅ GPS sensor ingestion successful: {result['message']}"
                    )
                    return True
                else:
                    error = await resp.text()
                    logger.error(
                        f"❌ GPS sensor ingestion failed: {resp.status} - {error}"
                    )
                    return False
        except Exception as e:
    ...
    async def test_image_ingestion(self) -> bool:
        ...
        try:
            async with self.session.post(
                f"{INGESTION_API_URL}/images/upload", json=SAMPLE_PAYLOADS["image"]
            ) as resp:
                if resp.status == 200:
                    result = await resp.json()
    ...
    async def test_note_ingestion(self) -> bool:
        ...
        try:
            async with self.session.post(
                f"{INGESTION_API_URL}/notes/upload", json=SAMPLE_PAYLOADS["note"]
            ) as resp:
                if resp.status == 200:
                    result = await resp.json()
    ...
    async def test_kafka_topic_existence(self) -> bool:
        ...
        try:
            ...
            if not missing_topics:
                logger.info(
                    f"✅ All required Kafka topics exist ({len(expected_topics)} topics)"
                )
                return True
            else:
    ...
    async def monitor_kafka_messages(
        self, topics: list, duration: int = 30
    ) -> Dict[str, int]:
                ...
                try:
                    ...
                    for topic_partition, messages in msg_pack.items():
                        ...
                        for message in messages:
                            logger.debug(
                                f"📨 Message on {topic}: {message.value[:100]}..."
                            )
