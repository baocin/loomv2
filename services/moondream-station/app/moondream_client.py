...
class MoondreamClient:
    ...
    async def _make_request(
        ...
        try:
            ...
            if e.response.status_code == 503 and retries < self.max_retries:
                ...
                return await self._make_request(endpoint, method, data, files, retries + 1)
            logger.error("HTTP error from Moondream Station", status=e.response.status_code, error=str(e))
            raise
    ...
    async def caption_image(self, image_data: str) -> str:
        ...
        try:
            ...
            files = {
                "image": ("image.jpg", image_bytes, mime_type)
            }
    ...
    async def extract_text(self, image_data: str) -> List[OCRBlock]:
        ...
        query = "What text can you read in this image? Please list all text you can see."
        response = await self.query_image(image_data, query)
