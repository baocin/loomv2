...
class TestMoondreamClient:
    ...
    def test_init(self, moondream_client):
        ...
        assert (
            moondream_client.base_url
            == f"{settings.moondream_host}{settings.moondream_api_path}"
        )
    ...
    async def test_make_request_success(
        self, mock_client_class, moondream_client, mock_httpx_response
    ):
    ...
    async def test_make_request_retry_on_503(self, mock_client_class, moondream_client):
        ...
        mock_response_503.raise_for_status.side_effect = Exception(
    ...
    def test_prepare_image_with_resize(
        self, moondream_client, sample_large_image_base64
    ):
        """Test image preparation with resizing."""
        image_bytes, mime_type = moondream_client._prepare_image(
            sample_large_image_base64
        )
    ...
    async def test_caption_image(
        self, mock_request, moondream_client, sample_image_base64
    ):
        ...
        mock_request.assert_called_once_with(
            "caption", files=mock_request.call_args[1]["files"]
        )
    ...
    async def test_query_image(
        ...
        result = await moondream_client.query_image(
            sample_image_base64, "How many cats are in this image?"
        )
    ...
    async def test_detect_objects(
        self, mock_query, moondream_client, sample_image_base64
    ):
    ...
    @patch("app.moondream_client.MoondreamClient.query_image")
    async def test_analyze_visual_features(
        self, mock_query, moondream_client, sample_image_base64
    ):
        """Test visual feature analysis."""
        mock_query.return_value = (
            "The image is bright with high contrast and sharp details"
        )
