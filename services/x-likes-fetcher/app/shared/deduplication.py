...
class ContentHasher:
    ...
    def generate_twitter_hash(self, tweet_id: str, url: Optional[str] = None) -> str:
        """
        Generate hash for Twitter/X posts using tweet ID as primary identifier.
        Tweet IDs are unique and immutable, making them ideal for deduplication.
        """
        if not tweet_id:
            raise ValueError("Tweet ID is required for Twitter hash generation")

        # Normalize tweet ID (remove any non-alphanumeric chars)
        tweet_id_norm = self.normalize_id(tweet_id)

        # Use tweet ID as the primary component
        content = f"twitter:{self.VERSION}:id:{tweet_id_norm}"
    ...
    def generate_generic_hash(self, data_type: str, **kwargs) -> str:
        ...
        hash_methods = {
            ...
            "twitter": self.generate_twitter_hash,
        }
