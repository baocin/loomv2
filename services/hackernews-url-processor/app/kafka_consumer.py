...
class KafkaConsumer:
    def __init__(self):
        ...
        self.group_id = os.getenv("KAFKA_CONSUMER_GROUP", "hackernews-url-processor")
    ...
    async def consume_messages(self, topic: str) -> AsyncGenerator[Dict[str, Any], None]:
                ...
                if message_pack:
                    for topic_partition, messages in message_pack.items():
                        for message in messages:
                            try:
                                logging.debug(f"Received message from {topic_partition.topic} at offset {message.offset}")
                                yield message.value
                            except Exception as e:
