...
class TestKafkaIntegration:
    ...
    def test_kafka_consumer_configuration(self, mock_getenv, mock_consumer_class, mock_producer_class):
        ...
        with patch("app.main.ocr_processor") as mock_processor:
            mock_processor.process_image.return_value = {"success": True, "ocr_text": "", "description": ""}
    ...
    @patch("app.main.KafkaConsumer")
    @patch("app.main.os.getenv")
    def test_kafka_producer_configuration(self, mock_getenv, mock_consumer_class, mock_producer_class):
    ...
    def test_kafka_consumer_no_processor(self, mock_logging, mock_consumer_class, mock_producer_class):
        """Test Kafka consumer handles missing OCR processor."""
        with patch("app.main.ocr_processor", None):
            kafka_consumer_thread()
            
            mock_logging.error.assert_called_with("OCR processor not initialized for Kafka consumer")
    ...
    def test_message_processing_success(self, mock_time, mock_consumer_class, mock_producer_class):
    ...
    @patch("app.main.KafkaProducer") 
    @patch("app.main.KafkaConsumer")
    def test_message_processing_with_base64_prefix(self, mock_consumer_class, mock_producer_class):
    ...
    @patch("app.main.KafkaConsumer")
    @patch("app.main.logging")
    def test_message_processing_no_image_data(self, mock_logging, mock_consumer_class, mock_producer_class):
        """Test handling message without image data."""
        test_message = {
            "data": {},  # No image_data field
            "trace_id": "test-trace"
        }
        mock_kafka_message = MagicMock()
        mock_kafka_message.value = test_message
    ...
    @patch("app.main.KafkaConsumer")
    @patch("app.main.logging")
    def test_message_processing_ocr_error(self, mock_logging, mock_consumer_class, mock_producer_class):
    ...
    @patch("app.main.KafkaConsumer")
    @patch("app.main.logging")
    def test_message_processing_exception(self, mock_logging, mock_consumer_class, mock_producer_class):
