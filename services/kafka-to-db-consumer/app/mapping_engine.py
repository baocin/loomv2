...
class MappingEngine:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Loaded topic mapping configuration",
    ...
    def map_message_to_record(
        ...
        try:
            ...
            upsert_key = topic_config.get(
                "upsert_key", defaults.get("upsert_key", "trace_id")
            )
            ...
            for source_field, target_column in field_mappings.items():
                ...
                if value is not None:
                    ...
                    if target_column in data_types:
                        value = self._convert_data_type(
                            value, data_types[target_column]
                        )

                    # Apply transformations
                    if target_column in transforms:
                        value = self._apply_transformation(
                            value, transforms[target_column]
                        )
            ...
            if array_mappings:
                array_records = self._process_array_mappings(
                    message, array_mappings, record, topic_config
                )
                if array_records:
            ...
            logger.debug(
                "Successfully mapped message to record",
                topic=topic,
                table=table_name,
                trace_id=record.get("trace_id"),
            ...
            logger.error(
                "Failed to map message to record",
                topic=topic,
                error=str(e),
                message_keys=(
    ...
    def _process_array_mappings(
        self,
        message: Dict[str, Any],
        array_mappings: Dict[str, Any],
        ...
        try:
            ...
            logger.error(
                "Failed to process array mappings",
                error=str(e),
    ...
    def validate_config(self) -> List[str]:
        ...
        for topic_name, topic_config in topics.items():
            ...
            if "field_mappings" not in topic_config:
                errors.append(
                    f"Topic {topic_name}: missing required 'field_mappings' field"
                )
            ...
            if not isinstance(field_mappings, dict):
                errors.append(
                    f"Topic {topic_name}: 'field_mappings' must be a dictionary"
                )
            ...
            for field, data_type in data_types.items():
                if data_type not in valid_types:
                    errors.append(
                        f"Topic {topic_name}: invalid data type '{data_type}' for field '{field}'"
                    )
