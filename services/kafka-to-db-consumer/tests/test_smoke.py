...
class TestServiceSmoke:
    ...
    async def test_main_startup_shutdown(self):
        ...
        with patch('app.main.asyncpg.create_pool', new_callable=AsyncMock) as mock_create_pool, \
             patch('app.main.AIOKafkaConsumer') as mock_kafka_consumer_class, \
             patch('app.main.yaml.safe_load') as mock_yaml_load, \
            ...
            mock_yaml_load.return_value = {
                ...
                "topics": {
                    "device.sensor.gps.raw": {
                        ...
                        "required_fields": ["trace_id", "device_id", "timestamp", "data.latitude", "data.longitude"]
                    }
                }
            }
