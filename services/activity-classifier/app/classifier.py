...
class ActivityClassifier:
    ...
    def add_event(self, device_id: str, event_type: str, event_data: dict) -> Optional[ActivityClassification]:
        ...
        if last_time and (now - last_time).total_seconds() < settings.classification_window_seconds:
            return None
    ...
    def _clean_old_events(self, device_id: str):
        ...
        cutoff = datetime.utcnow() - timedelta(seconds=settings.classification_window_seconds * 2)
        self.event_buffer[device_id] = [
            e for e in self.event_buffer[device_id]
    ...
    def _analyze_motion_events(self, events: List[dict]) -> dict:
        ...
        return {
            ...
            'dominant_motion': max(motion_types.items(), key=lambda x: x[1])[0] if motion_types else None,
            'avg_acceleration': np.mean(avg_accelerations) if avg_accelerations else 0.0
        }
    ...
    def _analyze_gps_events(self, events: List[dict]) -> dict:
        ...
        for i in range(1, len(events)):
            ...
            if 'latitude' in prev and 'longitude' in prev and 'latitude' in curr and 'longitude' in curr:
                # Calculate distance
    ...
    def _determine_location_context(self, gps_events: List[dict]) -> Optional[LocationContext]:
