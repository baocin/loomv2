...
class EmailFetcher:
    ...
    def _load_accounts(self) -> List[Dict[str, Any]]:
        ...
        # Support up to 10 email accounts
        for i in range(1, 11):
            email_env = f"EMAIL_{i}"
            password_env = f"EMAIL_PASSWORD_{i}"
            imap_server_env = f"EMAIL_IMAP_SERVER_{i}"
            port_env = f"EMAIL_PORT_{i}"
            disabled_env = f"EMAIL_DISABLED_{i}"
            ...
            account = {
                ...
                "imap_server": os.getenv(imap_server_env),
                "port": int(os.getenv(port_env, 993)),
                "disabled": os.getenv(disabled_env, "false").lower() == "true"
            }
            
            if not account["disabled"] and account["password"] and account["imap_server"]:
    ...
    def _fetch_account_emails(self, account: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if unseen_messages[0]:
                ...
                logging.info(f"Found {len(email_ids)} new emails for {account['email']}")
    ...
    def _parse_email(self, mail, email_id: bytes, account: Dict[str, Any]) -> Dict[str, Any]:
