...
class NomicEmbeddingProcessor:
    ...
    async def _embed_text(self, text: str) -> list[float]:
        ...
        try:
            ...
            if len(text) > settings.max_text_length:
                ...
                logger.warning(
                    f"Text truncated to {settings.max_text_length} characters"
                )

            # Generate embedding in a thread to avoid blocking
            loop = asyncio.get_event_loop()
            embedding = await loop.run_in_executor(None, self._sync_embed_text, text)
    ...
    async def _embed_image(
        self, image_data: str, generate_description: bool = False
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            embedding = await loop.run_in_executor(None, self._sync_embed_image, image)
    ...
    async def process_message(
        self, message_data: dict[str, Any], source_topic: str
        ...
        try:
                ...
                if field in message_data and message_data[field]:
                    ...
                    result = {
                        ...
                        "metadata": {
                            ...
                            if k not in ["content", "text", "chunk_data", "image_data"]
                        },
                    }
            ...
            image_fields = [
                "image_data",
                "chunk_data",
            ]  # chunk_data for audio, but we skip that
            for field in image_fields:
                if (
                    field in message_data
                    and message_data[field]
                    and field == "image_data"
                ):
                    image_embedding, description = await self._embed_image(
                        message_data[field], generate_description=True
                    )
                    ...
                    result = {
                        ...
                        "metadata": {
                            k: v
                            for k, v in message_data.items()
                            if k not in ["image_data", "chunk_data"]
                        },
                    }
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if total_processed > 0:
            avg_processing_time = (
                self.processing_stats["total_processing_time"] / total_processed
            )
