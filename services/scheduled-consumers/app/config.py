...
class Settings(BaseSettings):
    ...
    kafka_client_id: str = Field(
        default="loom-scheduled-consumers", description="Kafka client ID"
    )
    kafka_default_partitions: int = Field(
        default=3, description="Default number of partitions for new topics"
    )
    ...
    scheduler_interval_seconds: int = Field(
        default=300,
        description="Base scheduler check interval in seconds",  # 5 minutes
    )
    max_concurrent_jobs: int = Field(
        default=5, description="Maximum concurrent scheduled jobs"
    )

    # External API configurations
    email_check_interval_minutes: int = Field(
        default=15, description="Email check interval in minutes"
    )
    calendar_sync_interval_minutes: int = Field(
        default=30, description="Calendar sync interval in minutes"
    )
    social_media_check_interval_minutes: int = Field(
        default=60, description="Social media check interval in minutes"
    )
    web_activity_check_interval_minutes: int = Field(
        default=10, description="Web activity check interval in minutes"
    )

    # API credentials (should be set via environment variables)
    twitter_bearer_token: str = Field(
        default="", description="Twitter/X API bearer token"
    )
    gmail_credentials_path: str = Field(
        default="", description="Gmail API credentials file path"
    )
    calendar_credentials_path: str = Field(
        default="", description="Calendar API credentials file path"
    )

    # Browser data paths
    chrome_history_path: str = Field(
        default="", description="Path to Chrome browser history database"
    )
    firefox_history_path: str = Field(
        default="", description="Path to Firefox browser history database"
    )
