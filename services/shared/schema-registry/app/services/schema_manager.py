...
class SchemaManager:
    ...
    async def _load_schema_file(self, schema_file: Path) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to load schema file", file=str(schema_file), error=str(e)
            )
            raise
    ...
    async def validate(
        ...
        try:
            ...
            logger.error(
                "Schema validation failed", schema_name=schema_name, error=str(e)
            )
    ...
    async def _update_usage_stats(self, schema_name: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to update usage stats", schema_name=schema_name, error=str(e)
            )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_used_schemas": sorted(
                self.schema_stats.items(), key=lambda x: x[1], reverse=True
            )[:10],
            "cache": cache_stats,
        }
