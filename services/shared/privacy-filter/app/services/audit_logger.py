...
class AuditLogger:
    ...
    async def get_audit_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_entities_processed": sum(
                e.entities_processed for e in self.audit_entries
            ),
    ...
    async def clear_audit_trail(self, before_timestamp: Optional[float] = None):
        ...
        if before_timestamp:
            ...
            self.audit_entries = [
                e for e in self.audit_entries if e.timestamp >= before_timestamp
            ]
            logger.warning(
            ...
            self.audit_entries = []
            logger.warning(
                "Complete audit trail cleared", entries_cleared=entries_cleared
            )
