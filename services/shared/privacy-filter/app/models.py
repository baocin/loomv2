...
class PIIDetectionRequest(BaseModel):
    ...
    entity_types: Optional[List[EntityType]] = Field(
        default=None, description="Specific entity types to detect (all if None)"
    )
    language: str = Field(default="en", description="Language code")
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional context for auditing"
    )
...
class FilterRequest(BaseModel):
    ...
    replacement_strategy: ReplacementStrategy = Field(
        default=ReplacementStrategy.REDACT, description="How to replace detected PII"
    )
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional context for auditing"
    )
...
class FilterResponse(BaseModel):
    ...
    entities_found: List[PIIEntity] = Field(
        description="PII entities that were filtered"
    )
...
class AnonymizeRequest(BaseModel):
    ...
    entity_types: Optional[List[EntityType]] = Field(
        default=None, description="Specific entity types to anonymize (all if None)"
    )
    preserve_format: bool = Field(
        default=True, description="Whether to preserve original format"
    )
    seed: Optional[int] = Field(
        default=None, description="Random seed for consistent fake data"
    )
    return_mapping: bool = Field(
        default=False, description="Whether to return anonymization mapping"
    )
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional context for auditing"
    )
