...
class CalendarInjest:
    def __init__(self, DB):
        ...
        all_accounts = [
            {
                "url": os.getenv("CALDAV_URL_1"),
                "username": os.getenv("CALDAV_USERNAME_1"),
                "password": os.getenv("CALDAV_PASSWORD_1"),
                "disabled": os.getenv("CALDAV_DISABLED_1"),
            },
            {
                "url": os.getenv("CALDAV_URL_2"),
                "username": os.getenv("CALDAV_USERNAME_2"),
                "password": os.getenv("CALDAV_PASSWORD_2"),
                "disabled": os.getenv("CALDAV_DISABLED_2"),
            },
            {
                "url": os.getenv("CALDAV_URL_3"),
                "username": os.getenv("CALDAV_USERNAME_3"),
                "password": os.getenv("CALDAV_PASSWORD_3"),
                "disabled": os.getenv("CALDAV_DISABLED_3"),
            },
        ]
        self.accounts = [
        ...
        try:
            ...
            for calendar in calendars:
                ...
                for event in events:
                    ...
                    event_id = (
                        f"{calendar.url}_{event.vobject_instance.vevent.uid.value}"
                    )
                    if self.event_exists(
                        event_id, f"{account['username']} - {calendar.name}"
                    ):
                    ...
                    try:
                        ...
                        calendar_data = {
                            ...
                            "description": (
                                event.vobject_instance.vevent.description.value
                                if hasattr(event.vobject_instance.vevent, "description")
                                else ""
                            ),
                            "location": (
                                event.vobject_instance.vevent.location.value
                                if hasattr(event.vobject_instance.vevent, "location")
                                else ""
                            ),
                            "pull_id": pull_id,
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if data:
                                        ...
                                        logging.info(
                                            f"GPS point for location '{calendar_data['location']}': {calendar_data['gps_point']}"
                                        )
                                else:
                                    logging.error(
                                        f"Failed to fetch GPS point for location '{calendar_data['location']}' with status code: {response.status_code}"
                                    )
                            except Exception as e:
                                logging.error(
                                    f"Error fetching GPS point for location '{calendar_data['location']}': {e}"
                                )

                        # Generate embedding for the event summary
                        calendar_data["embedding"] = self.embedding_service.embed_text(
                            [
                        ...
                        logging.error(
                            f"Error processing event {event_id} for {account['username']}: {e}"
                        )
                        continue
    ...
    def insert_calendar_data(self, calendar_data):
        if self.event_exists(
            calendar_data["event_id"], calendar_data["source_calendar"]
        ):
            sql = """
    ...
    def event_exists(self, event_id, source_calendar):
        sql = (
            "SELECT 1 FROM calendar_events WHERE event_id = %s AND source_calendar = %s"
        )
        cursor = self.db.cursor()
