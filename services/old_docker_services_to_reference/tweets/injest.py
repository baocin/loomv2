...
class TweetInjest:
    ...
    async def login(self):
        ...
        if os.path.exists(x_session_file):
            ...
            if login_button:
                ...
                print(
                    "Tweet_injest Login button not found, entering phone number and password"
                )
                await human_like_typing(
                    page,
                    'div[aria-labelledby="modal-header"] input[name="text"]',
                    os.getenv("X_PHONE_NUMBER"),
                )
                await page.click(
                    'div[aria-labelledby="modal-header"] span:has-text("Next")'
                )
    ...
    def insert_tweet(self, tweet_data):
        try:
            ...
            if existing_tweet:
                logging.info(
                    f"Tweet with link {tweet_url} already exists. Skipping insertion."
                )
                cursor.close()
                return
    ...
    async def insert_tweet_extended(self, tweet_data):
        try:
            ...
            tweet_json = json.dumps(
                {k: v for k, v in tweet_data.items() if k != "image_data"}
            )
            tweet_sql = """
            UPDATE tweets
            SET tweet_json = %s
    ...
    async def scrape_tweets(self):
        ...
        try:
            ...
            logging.warning(
                f"Network idle timeout reached, continuing with scraping: {e}"
            )
        ...
        while True:
            ...
            new_tweets = [
                ...
                if tweet["tweetLink"] not in existing_tweets
            ]
            ...
            if no_new_tweets_count > wait_for_x_tweets:
                logging.info(
                    f"No new tweets found for {wait_for_x_tweets} consecutive times. Stopping."
                )
                break
    ...
    async def screenshot_all_tweets(self):
        ...
        for tweet_id, tweet_url in tweets:
            ...
            cursor.execute(
                "SELECT 1 FROM tweet_images WHERE tweet_id = %s", (tweet_id,)
            )
