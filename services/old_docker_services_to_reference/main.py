...
class TaskManager:
    ...
    async def run_task(self, task_info):
        if not task_info["lock"].acquire(blocking=False):
            logging.info(
                f"{task_info['task'].__name__} already in progress, skipping this run."
            )
            return
...
if __name__ == "__main__":
    ...
    manager.add_task(
        seconds_in_hour * 0.5, fetch_calendar_task, manager.fetch_calendar_lock
    )
    manager.add_task(60, record_server_stats_task, manager.record_server_stats_lock)
    manager.add_task(
        seconds_in_hour * 12, fetch_contacts_task, manager.fetch_contacts_lock
    )
