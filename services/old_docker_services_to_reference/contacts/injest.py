...
class ContactsInjest:
    def __init__(self, DB):
        ...
        all_accounts = [
            {
                "url": os.getenv("CARDDAV_URL_1"),
                "username": os.getenv("CARDDAV_USERNAME_1"),
                "password": os.getenv("CARDDAV_PASSWORD_1"),
                "disabled": os.getenv("CARDDAV_DISABLED_1"),
            },
            {
                "url": os.getenv("CARDDAV_URL_2"),
                "username": os.getenv("CARDDAV_USERNAME_2"),
                "password": os.getenv("CARDDAV_PASSWORD_2"),
                "disabled": os.getenv("CARDDAV_DISABLED_2"),
            },
            {
                "url": os.getenv("CARDDAV_URL_3"),
                "username": os.getenv("CARDDAV_USERNAME_3"),
                "password": os.getenv("CARDDAV_PASSWORD_3"),
                "disabled": os.getenv("CARDDAV_DISABLED_3"),
            },
        ]
        self.accounts = [
        ...
        try:
            ...
            if response.status_code == 207:
                ...
                for response in root.findall("{DAV:}response"):
                    ...
                    vcard_response = requests.get(
                        vcard_url, auth=(account["username"], account["password"])
                    )
                    if vcard_response.status_code == 200:
                        ...
                        logging.error(
                            f"Failed to fetch vCard for href: {vcard_url} with status code: {vcard_response.status_code}"
                        )
                self.db.commit()  # Commit all inserts once the account is done
            else:
                logging.error(
                    f"Failed to fetch contacts for {account['username']}: {response.status_code}"
                )
        ...
        try:
            ...
            for line in lines:
                if line.startswith("FN:"):
                    ...
                    contact_data["email"] = (
                        email_parts[1] if len(email_parts) > 1 else ""
                    )
                    ...
                    contact_data["created_at"] = datetime.strptime(
                        line[4:], "%Y%m%dT%H%M%SZ"
                    )

            if contact_data:
                contact_data["last_contacted"] = datetime.now()
                contact_data["last_seen_timestamp"] = datetime.now()
                contact_data["embedding"] = (
                    self.embedding_service.embed_text([contact_data["raw_vcard"]])[0]
                    if "raw_vcard" in contact_data
                    else None
                )
                contact_data[
                    "face_images"
                ] = None  # Assuming face images are not available at this point
                contact_data[
                    "last_seen_location"
                ] = None  # Assuming last seen location is not available at this point
