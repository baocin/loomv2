...
class TestKafkaConsumerManager:
    ...
    async def test_start_success(
        self, mock_producer_class, mock_consumer_class, kafka_consumer_manager
    ):
    ...
    async def test_process_text_message(
        self, kafka_consumer_manager, mock_ollama_client, sample_text_message
    ):
    ...
    async def test_process_image_message(
        self, kafka_consumer_manager, mock_ollama_client, sample_image_message
    ):
    ...
    async def test_process_audio_message(
        self, kafka_consumer_manager, mock_ollama_client, sample_audio_message
    ):
    ...
    async def test_process_multimodal_message_text_topic(
        self, kafka_consumer_manager, sample_text_message
    ):
        """Test routing text topic to text processor."""
        message_dict = sample_text_message.model_dump(mode="json")

        result = await kafka_consumer_manager.process_multimodal_message(
            message_dict, "media.text.transcribed.words"
        )

        assert result is not None
        assert result.analysis_type == "text_analysis"

    async def test_process_multimodal_message_image_topic(
        self, kafka_consumer_manager, sample_image_message
    ):
        """Test routing image topic to image processor."""
        message_dict = sample_image_message.model_dump(mode="json")

        result = await kafka_consumer_manager.process_multimodal_message(
            message_dict, "device.image.camera.raw"
        )

        assert result is not None
        assert result.analysis_type == "image_analysis"

    async def test_process_multimodal_message_audio_topic(
        self, kafka_consumer_manager, sample_audio_message
    ):
        """Test routing audio topic to audio processor."""
        message_dict = sample_audio_message.model_dump(mode="json")

        result = await kafka_consumer_manager.process_multimodal_message(
            message_dict, "media.audio.vad_filtered"
        )

        assert result is not None
        assert result.analysis_type == "audio_analysis"

    async def test_process_multimodal_message_unknown_topic(
        self, kafka_consumer_manager, sample_text_message
    ):
        ...
        result = await kafka_consumer_manager.process_multimodal_message(
            message_dict, "unknown.topic.name"
        )
    ...
    async def test_process_message_ollama_error(
        self, kafka_consumer_manager, mock_ollama_client, sample_text_message
    ):
...
class TestKafkaMessageProcessing:
    ...
    @patch("app.kafka_consumer.AIOKafkaConsumer")
    @patch("app.kafka_consumer.AIOKafkaProducer")
    async def test_consume_messages_success(
        self,
        mock_producer_class,
        mock_consumer_class,
    ...
    @patch("app.kafka_consumer.AIOKafkaConsumer")
    @patch("app.kafka_consumer.AIOKafkaProducer")
    async def test_consume_messages_processing_failure(
        self, mock_producer_class, mock_consumer_class, kafka_consumer_manager
    ):
