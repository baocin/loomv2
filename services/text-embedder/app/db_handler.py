...
class DatabaseHandler:
    ...
    async def create_tables(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_emails_timestamp ON emails_with_embeddings (timestamp DESC)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_emails_sender ON emails_with_embeddings (sender_email)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_emails_device ON emails_with_embeddings (device_id)")
            
            # Create indexes for twitter table
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_twitter_timestamp ON twitter_likes_with_embeddings (timestamp DESC)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_twitter_author ON twitter_likes_with_embeddings (author_username)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_twitter_device ON twitter_likes_with_embeddings (device_id)")
    ...
    async def store_email_with_embedding(
        ...
        async with self.pool.acquire() as conn:
            ...
            row = await conn.fetchrow(
                ...
                email_data.get("device_id"),
                self._parse_timestamp(email_data.get("timestamp")),
    ...
    async def update_twitter_screenshot(
        ...
        async with self.pool.acquire() as conn:
            await conn.execute("""
                ...
                WHERE tweet_id = $3
            """, screenshot_url, json.dumps(extracted_content), tweet_id)
