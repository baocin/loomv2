...
class GeocodingService:
    def __init__(self):
        self.geocoder = OpenCageGeocode(settings.opencage_api_key) if settings.opencage_api_key else None
    ...
    def _find_cached_location(self, session: Session, latitude: float, longitude: float) -> Optional[CachedGeocoding]:
        ...
        lon_delta = settings.cache_radius_meters / (111000.0 * abs(latitude + 0.001))  # Avoid division by zero
        
        cached_locations = session.query(CachedGeocoding).filter(
            and_(
                CachedGeocoding.latitude.between(latitude - lat_delta, latitude + lat_delta),
                CachedGeocoding.longitude.between(longitude - lon_delta, longitude + lon_delta)
            )
        ).all()
        ...
        for location in cached_locations:
            dist = self._calculate_distance(latitude, longitude, location.latitude, location.longitude)
            if dist <= settings.cache_radius_meters and dist < min_distance:
                min_distance = dist
                closest = location
        
        if closest:
            logger.debug("Found cached location", 
                        distance_meters=min_distance,
                        cached_id=closest.id)
        
        return closest
    
    def _should_process_location(self, session: Session, latitude: float, longitude: float) -> bool:
        ...
        for cached in all_cached:
            dist = self._calculate_distance(latitude, longitude, cached.latitude, cached.longitude)
            if dist <= settings.min_distance_meters:
                logger.debug("Location too close to cached point",
    ...
    def geocode_location(self, session: Session, latitude: float, longitude: float) -> Optional[Dict[str, Any]]:
        ...
        if cached:
            ...
            logger.info("Using cached geocoding result",
                       cached_id=cached.id,
        ...
        if not self._should_process_location(session, latitude, longitude):
            logger.debug("Skipping geocoding for location", 
                        latitude=latitude, 
                        longitude=longitude)
            return None
        ...
        try:
            ...
            logger.info("Making OpenCage API call",
                       latitude=latitude,
                       longitude=longitude)
            ...
            logger.info("Geocoded and cached location",
