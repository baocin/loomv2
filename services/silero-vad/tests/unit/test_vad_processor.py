...
class TestVADProcessor:
    ...
    def test_process_audio_sync_mono(self):
        ...
        processor.get_speech_timestamps = Mock(
            return_value=[{"start": 160, "end": 480, "confidence": 0.95}]
        )

        # Create test audio data (mono, 16-bit PCM)
        audio_data = np.random.randint(
            -32768, 32767, size=1600, dtype=np.int16
        ).tobytes()
    ...
    def test_process_audio_sync_stereo(self):
        ...
        audio_data = np.random.randint(
            -32768, 32767, size=3200, dtype=np.int16
        ).tobytes()
